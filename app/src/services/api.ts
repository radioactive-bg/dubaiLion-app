...
export const redeemCard = async (payload: RedeemCardPayload): Promise<ApiResponse> => {
  try {
    ...
    await storeRedemptionData({
      tiktokUsername: localStorage.getItem('currentTiktokUsername') || 'unknown',
...
const storeRedemptionData = async (data: any): Promise<void> => {
  try {
    const previousEntries = JSON.parse(localStorage.getItem('redemptionEntries') || '[]');
