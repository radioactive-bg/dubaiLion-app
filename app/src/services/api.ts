...
const ZOHO_TOKEN = import.meta.env.VITE_ZOHO_FORMS_TOKEN!;
const ZOHO_FORM = import.meta.env.VITE_ZOHO_FORMS_LINK!;
const ZOHO_ENDPOINT = `https://forms.zoho.com/api/v2/forms/${ZOHO_FORM}/records`;

const API_URL =
  "https://proxy.duegate.com/staging/diistributor-crm/v1/wallets/1/credit";
const ACCESS_TOKEN =
...
export async function sendToZohoForms(payload: {
  tiktokUsername: string;
  cardSerialNumber: string;
  cvv: string;
}) {
  if (!ZOHO_TOKEN || !ZOHO_FORM) {
    throw new Error("Missing Zoho Forms API token or form link");
  }

  const url = ZOHO_ENDPOINT;

  // Build the record body
  const body = {
    data: [
      {
        // these keys MUST match the "Field Link Name" in Zoho Forms exactly:
        TikTok_Username: payload.tiktokUsername,
        Card_Serial_Number: payload.cardSerialNumber,
        CVV: payload.cvv,
      },
    ],
  };
  ...
  // Zoho returns 201 on success, with the new record IDs
  if (response.status !== 201) {
    throw new Error(`Zoho Forms API error: ${response.status}`);
  }
...
export const redeemCard = async (
  ...
  // 1) Send to Zoho Forms
  try {
    await sendToZohoForms({
      tiktokUsername,
      cardSerialNumber: payload.data.number,
      cvv: payload.data.cvv,
    });
  } catch (zohoErr) {
    console.warn("Zoho Forms submission failed:", zohoErr);
    // decide whether to bail here or continue
  }
  try {
